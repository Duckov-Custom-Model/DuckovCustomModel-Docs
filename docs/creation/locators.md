...
模型 Prefab 的子对象中需要包含以下名称的 `Transform` 作为定位锚点，缺少锚点会导致无法显示指定位置的物品或装备，设置锚点的`Transform`缩放为0来隐藏物品或装备：
...
可以用来调整角色在骑乘状态下的位置和朝向，注意该锚点是放在载具上的，不是放在人物上的。

可以配合[IsControllingVehicle](../animator/bool-params.md#控制相关参数)参数使用调整角色在乘骑时的动作
